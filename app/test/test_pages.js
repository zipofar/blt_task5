...
describe('API Pages', () => {
  ...
  /*
  before(async () => {
    await chai.request.agent(server).post('/auth/login').type('json').send(userCredentials).end();
  });
  */
  ...
  it('should return first page', async () => {
    ...
    const resLogin = await agent.post('/auth/login').type('json').send(userCredentials);
    const jwt = resLogin.body.data;
  ...
  /*
  it('should return second page (next 5 record)', (done) => {
    chai
      .request(server)
      .get('/pages/?page=2')
      .end((err, res) => {
